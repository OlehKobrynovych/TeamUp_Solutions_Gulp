...
function html() {
  return gulp.src('./src/*.html')
    // .pipe(rigger())  // для вставки файлів у файл (розбиття на компоненти)     //= template/footer.html
...
function js() {
  return gulp.src([ 
    ...
    // .pipe(jsmin())                  // видаляє пробіли
    // .pipe(rename({suffix: '.min'}))
    .pipe(gulp.dest('./dist/js'))
    // .pipe(notify("js OK!"))    // повідомлення 
    .pipe(browserSync.stream());   // перезагрузка бравзере
}
...
function img() {
  return gulp.src('./src/assets/img/**/*')
    .pipe(newer('./dist/assets/img/'))       // перевірка наявності картинки
...
exports.default = gulp.parallel(html, scss, img, fonts, watch);
